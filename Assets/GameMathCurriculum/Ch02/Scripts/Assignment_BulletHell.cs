...
public class Assignment_BulletHell : MonoBehaviour
{
    ...
    private Vector3 CalculateCircleDirection(int index, int total)
    {
        ...
        float angleDegree = (index * angleSpacing + fireInterval) % 360;
    ...
    { 
        ...
        float angleSpacing = 360f / total;
        float finalAngle = (index * angleSpacing) + SpiralAngle;

        Quaternion rotation = Quaternion.Euler(0f, finalAngle, 0f);
        if (index == total-1)
        {
