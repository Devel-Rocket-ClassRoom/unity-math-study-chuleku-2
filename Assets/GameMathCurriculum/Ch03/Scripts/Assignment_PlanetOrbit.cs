...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private float currentPlanetAngle;
    private float currentSatelliteAngle;
    ...
    private void Update()
    {
        ...
        currentPlanetAngle += planetOrbitSpeed * Time.deltaTime;

        Matrix4x4 planetOrbitMatrix = Matrix4x4.TRS(orbitCenter, Quaternion.Euler(0f, currentPlanetAngle, 0f), Vector3.one);

        transform.position = planetOrbitMatrix.MultiplyPoint(new Vector3(planetOrbitRadius, 1f, 0f));

        currentSatelliteAngle += satelliteOrbitSpeed * Time.deltaTime;

        Matrix4x4 satelliteOrbitMatrix = Matrix4x4.TRS(transform.position, Quaternion.Euler(0f, currentSatelliteAngle, 0f), Vector3.one);
