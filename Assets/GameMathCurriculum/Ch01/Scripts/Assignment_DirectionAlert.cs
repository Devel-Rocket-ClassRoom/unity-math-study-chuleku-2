...
/*    private Direction GetDirection(Transform enemy)
    {
        ...
        if (toTarget.magnitude > alertRange)
        {
            return Direction.None;
        }
        toTarget.y = 0f;
        Vector3 toTargetNormalIzed = toTarget.normalized;
        Vector3 crossProduct = Vector3.Cross(transform.forward, toTargetNormalIzed);
        float threshold = 0.45f;
        float crossY = crossProduct.y;
        float dotProductValue = Vector3.Dot(transform.forward, toTargetNormalIzed);
        float halfFovCos = 0f;

        if (dotProductValue > halfFovCos)
        {
            //앞
            if (crossY > threshold)
            {
